...
export const UserProfile = () => {
  ...
  // Try multiple ways to extract wallet address for better compatibility
  const walletAddress =
    user?.wallet?.address ||
  ...
  return (
    <div className={`min-h-screen p-4 md:p-8 transition-colors ${isDark ? 'bg-[#1a1a1a]' : 'bg-[#f0f0f0]'}`}>
      <div className="max-w-6xl mx-auto">
        ...
        {/* Profile Card */}
        <div className={`border-4 sm:border-6 md:border-8 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] sm:shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] md:shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] mb-6 sm:mb-8 ${isDark ? 'bg-[#2a2a2a] text-white' : 'bg-white text-black'}`}>
          <div className="grid grid-cols-1 md:grid-cols-3">

            {/* Left: Avatar & Identity */}
            <div className={`p-4 sm:p-6 md:p-8 flex flex-col items-center justify-center border-b-4 sm:border-b-6 md:border-b-8 md:border-b-0 md:border-r-4 sm:md:border-r-6 md:border-r-8 border-black bg-gradient-to-br from-blue-600 to-blue-800 text-white`}>
              <div
                onClick={() => setShowAvatarSelector(true)}
                className="w-24 h-24 sm:w-32 sm:h-32 md:w-40 md:h-40 border-4 sm:border-6 md:border-8 border-white rounded-full overflow-hidden flex items-center justify-center bg-white mb-4 sm:mb-6 cursor-pointer hover:border-yellow-300 transition-all group relative"
              >
                <AgentAvatar
              ...
              <h1 className="text-xl sm:text-2xl md:text-3xl font-black uppercase text-center leading-none">OPERATOR</h1>
              <div className="mt-2 font-mono text-[10px] sm:text-xs bg-black/30 px-2 sm:px-3 py-1 rounded-full flex items-center gap-1 sm:gap-2 max-w-full overflow-hidden">
                <Wallet size={12} className="flex-shrink-0" />
                <span className="truncate">
                  {walletAddress ? `${walletAddress.slice(0, 4)}...${walletAddress.slice(-4)}` : 'CONNECTING...'}
                </span>
              </div>
              {currentAgent ? (
                <button
            ...
            {/* Right: Stats & Info */}
            <div className="col-span-2 p-4 sm:p-6 md:p-8 flex flex-col justify-between">
              <div>
                <h2 className="text-sm sm:text-base md:text-xl font-black uppercase mb-4 sm:mb-6 flex items-center gap-2 opacity-50">
                  // OPERATOR_DOSSIER
                </h2>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 md:gap-4 mb-6 sm:mb-8">
                  <div className={`border-3 sm:border-4 border-black p-2 sm:p-3 md:p-4 text-center ${isDark ? 'bg-black/30' : 'bg-[#f0f0f0]'}`}>
                    <div className="text-2xl sm:text-3xl font-black text-[#FF00FF]">{avatars.length}</div>
                    <div className="text-[9px] sm:text-[10px] font-bold uppercase opacity-60">Avatars</div>
                  </div>
                  <div className={`border-3 sm:border-4 border-black p-2 sm:p-3 md:p-4 text-center ${isDark ? 'bg-black/30' : 'bg-[#f0f0f0]'}`}>
                    <div className="text-2xl sm:text-3xl font-black text-[#0052FF]">{intel.length}</div>
                    <div className="text-[9px] sm:text-[10px] font-bold uppercase opacity-60">Intel</div>
                  </div>
                  <div className={`border-3 sm:border-4 border-black p-2 sm:p-3 md:p-4 text-center ${isDark ? 'bg-black/30' : 'bg-[#f0f0f0]'}`}>
                    <div className="text-2xl sm:text-3xl font-black text-[#FFD700]">{bonds.length}</div>
                    <div className="text-[9px] sm:text-[10px] font-bold uppercase opacity-60">Bonds</div>
                  </div>
                  <div className={`border-3 sm:border-4 border-black p-2 sm:p-3 md:p-4 text-center ${isDark ? 'bg-black/30' : 'bg-[#f0f0f0]'}`}>
                    <div className="text-2xl sm:text-3xl font-black text-[#00FF00]">100%</div>
                    <div className="text-[9px] sm:text-[10px] font-bold uppercase opacity-60">Humanity</div>
                  </div>
                </div>
              </div>

              {/* Tabs */}
              <div className="flex border-3 sm:border-4 border-black">
                <button
                  onClick={() => setActiveTab('vault')}
                  className={`flex-1 py-2 sm:py-3 md:py-4 font-black uppercase text-xs sm:text-sm flex items-center justify-center gap-1 sm:gap-2 transition-all ${activeTab === 'vault' ? 'bg-black text-white' : 'hover:bg-black/10'}`}
                >
                  <Database size={14} className="sm:w-4 sm:h-4" />
                  <span className="hidden xs:inline">INTEL_</span>VAULT
                </button>
                <div className="w-1 bg-black" />
                <button
                  onClick={() => setActiveTab('bonds')}
                  className={`flex-1 py-2 sm:py-3 md:py-4 font-black uppercase text-xs sm:text-sm flex items-center justify-center gap-1 sm:gap-2 transition-all ${activeTab === 'bonds' ? 'bg-black text-white' : 'hover:bg-black/10'}`}
                >
                  <Users size={14} className="sm:w-4 sm:h-4" />
                  <span className="hidden xs:inline">ACTIVE_</span>BONDS
                </button>
              </div>
            </div>
          </div>
                ...
                avatars.map((avatar) => (
                  ...
                  >
                    ...
                    <div className="relative">
                      <div
                        className="overflow-hidden relative border-3 sm:border-4 border-black border-b-0 rounded-t-full flex items-start justify-center pt-2 sm:pt-3"
                        style={{
                          height: '120px',
                      ...
                      >
                        <div className="transform scale-90 group-hover:scale-95 transition-transform duration-500">
                          <AgentAvatar identifier={avatar.seed} size={100} style={avatar.style} />
                        </div>
            ...
            {bonds.map(handle => (
              ...
              >
                <div className="flex items-center gap-3 sm:gap-4">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 border-3 sm:border-4 border-current rounded-full overflow-hidden bg-white flex-shrink-0">
                    <AgentAvatar identifier={handle} size={48} />
                  </div>
          ...
          <div className={`max-w-4xl w-full max-h-[90vh] overflow-auto border-8 border-black ${isDark ? 'bg-[#2a2a2a]' : 'bg-white'}`}>
            ...
            <div className="p-4 sm:p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
              ...
              <div
                ...
                className={`border-3 sm:border-4 border-black p-2 sm:p-3 cursor-pointer hover:bg-black hover:text-white transition-all ${
                  !currentAvatar ? 'bg-green-500 text-white' : ''
                }`}
              >
                <div className="aspect-square border-2 sm:border-3 border-black mb-2 overflow-hidden flex items-center justify-center bg-gray-100">
                  <AgentAvatar identifier={walletAddress || 'operator'} size={100} />
                </div>
                <p className="font-black uppercase text-[10px] sm:text-xs text-center">
                  DEFAULT
